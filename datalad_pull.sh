...
for i in 22671 70467 62003 72082 85305 52662 89475 42107 61373 89474 03748 84896 02811; do datalad get sub-ON${i}/ses-01/meg/*${task}*  sub-ON${i}/ses-01/anat/*T1w* sub-ON${i}/ses-01/meg/*noise*.ds; done
...
for i in 22671 70467 62003 72082 85305 52662 89475 42107 61373 89474 03748 84896 02811; do datalad unlock sub-ON${i}/ses-01/meg/*${task}* sub-ON${i}/ses-01/anat/*T1w* sub-ON${i}/ses-01/meg/*noise*  ; done
...
find ./sub-*/ses-01/anat -name '*T2star*' -exec rm -r {} \;

#Remove uneeded MEG datasets
for tmp in airpuff artifact gonogo haririhammer movie oddball rest sternberg; do if [ $tmp != $task ]; then find ./sub-* -name "*${tmp}*" -exec rm -r  {} \;; fi ; done

chmod -R 777 .bidsignore .git .gitattributes
rm -r .bidsignore .gitattributes .git
